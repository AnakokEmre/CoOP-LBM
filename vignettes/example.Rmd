...
Connectivity_CoOP = round(model2$alpha1%*%model2$pi%*%model2$alpha2,3)
Chao =round(mean(V)/(1 - F1/sum(R_obs) * (sum(R_obs)-1)*F1/((sum(R_obs)-1)*F1+2*(F2+1))),3)
Chao1 = round(estimateR(c(R_obs))["S.chao1"]/(n1*n2),3)
...
print(paste0("Connectivity | Complete matrix = " ,Connectivity_M ," | Observed matrix = ", Connectivity_V , " | LBM = ",Connectivity_LBM," | CoOP-LBM = ",Connectivity_CoOP," | Chao = ",Chao ," | Chao1 = ", Chao1 ) )
print(paste0("RMSE of row coverage | Chao = " ,round(RMSE_chao_L,3)," | Chao1 = ",round(RMSE_chao1_L,3) ," | CoOP-LBM = ", round(RMSE_CoOP_L,3) ))
print(paste0("RMSE of column coverage | Chao = " ,round(RMSE_chao_C,3)," | Chao1 = ",round(RMSE_chao1_C,3) ," | CoOP-LBM = ", round(RMSE_CoOP_C,3) ))
...
df3 = result[c("G","Connectivity_V","Connectivity_CoOP","Connectivity_Chao","Connectivity_Chao1")]
...
p7 <- ggplot(df3_long, aes(x=factor(G),y=value,fill=variable))+
  ...
  scale_fill_manual(labels = c("Observed matrix","CoOP-LBM","Chao coverage","Chao 1"),values= hue_pal()(4)[c(1,3,2,4)]) +
  labs(linetype="")+
